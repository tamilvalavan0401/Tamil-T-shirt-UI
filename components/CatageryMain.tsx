  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        // Set default category based on slug
        const slug = params.slug
        const category = categoriesData.find(cat => cat.seo_url === slug)
        if (category) {
          if (category.parent_id === 0) {
            // Main category
            ...
            // Subcategory
            const parentCategory = categoriesData.find(cat => cat.id === category.parent_id)
            if (parentCategory) {
              setSelectedMainCategory(parentCategory.seo_url)
              setSelectedFilters({ category: [parentCategory.seo_url, slug] })
            } else {
              setSelectedMainCategory(slug)
              setSelectedFilters({ category: [slug] })
            }
...
const setCategorySlug = (slug: string) => {
    router.push(`/category/${slug}`, { scroll: false }) // Client-side navigation without refresh
  }
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <div className="max-w-[90rem] mx-auto">
          <nav className="bg-white px-4 py-3 text-sm text-gray_text flex items-center">
            ...
            <Link href={`/category/${params.slug}`} className="hover:underline">
              {categories.find(cat => cat.seo_url === params.slug)?.name || "Category"}
            </Link>
          </nav>
